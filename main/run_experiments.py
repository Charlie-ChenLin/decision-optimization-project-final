...
def solve_direct(inst: Instance, time_limit: int = 300,
    ...
    C_op = gp.quicksum(
        p['rho']**(t-1) * gp.quicksum(
            dyc_op_cost * w1[z,t,s] + eyc_op_cost * w2[z,t,s] +
    ...
    C_eyc_operation_subsidy = gp.quicksum(
        p['rho']**(t-1) * gp.quicksum(
            p.get('eyc_operation_subsidy', 0.0) * w2[z,t,s]
            for z in Z) for t in T for s in S) / nS
    ...
    if model.status in (GRB.OPTIMAL, GRB.TIME_LIMIT):
        ...
        return model.ObjVal, solve_time, {
            ...
            'expected_dyc_emissions': expected_dyc_emissions,
            'expected_eyc_emissions': expected_eyc_emissions,
