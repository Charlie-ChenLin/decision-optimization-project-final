...
不确定性影响实验沿用小、中、大三类算例：
\[
(|Z|,|T|,|S|)=(3,12,5),(3,24,20),(5,36,50).
\]
其中 DEP 表示直接求解随机规划确定性等价模型，EV 表示用平均需求求解确定性模型后再代入随机场景评估。补贴灵敏度实验采用中规模算例，设置 $k=0,0.1,\ldots,1.0$；不确定性水平扩展实验设置需求变异系数 $\text{CV}=0.05,0.15,0.30$，并在每个 CV 下重复补贴率网格实验。
...
\subsection{真实港口规模算例与求解效率}

根据公开资料，上海港 2024 年集装箱吞吐量达到 5150.6 万 TEU\footnote{\url{https://www.portshanghai.com.cn/wenku/www/202504/071654076ni7.pdf}}，洋山四期自动化码头远期设计通过能力约为 630 万 TEU/年\footnote{\url{https://www.yidaiyilu.gov.cn/p/39135.html}}。为回应算例规模偏小的问题，本节进一步构造接近中国大型自动化集装箱码头吞吐量口径的算例：年吞吐量设为 630 万 TEU，规划期为 60 个月，需求场景数为 60，并将大型码头堆场聚合为 12 个模型作业区。该算例的完整 DEP 规模达到 652200 个变量和 263568 个约束，显著大于前述 $(5,36,50)$ 的大规模算例。

表\ref{tab:member3-china-scale} 比较了不同求解方法在该真实规模算例上的表现。Gurobi 完整 DEP 用作限时基准；EV、均值指导启发式、场景分解、代表情景法以及列生成/场景生成用于比较不同近似策略的效率和解质量。

\begin{table}[htbp]
\centering
\caption{真实港口规模算例下的求解效率对比}
\label{tab:member3-china-scale}
\begin{tabular}{lrrr}
\toprule
方法 & 目标值/$10^6$ & 时间(s) & 相对Gurobi差距 \\
\midrule
Gurobi完整DEP & 143.13 & 9.04 & 0.00\% \\
EV均值方案 & 144.09 & 1.25 & 0.67\% \\
代表情景法(12场景) & 143.13 & 2.47 & 0.00\% \\
代表情景法(20场景) & 143.13 & 3.57 & 0.00\% \\
均值指导启发式 & 144.09 & 5.07 & 0.67\% \\
场景分解 & 144.09 & 16.29 & 0.67\% \\
列生成/场景生成 & 143.13 & 27.40 & 0.00\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{../main/member3_china_scale_case.png}
\caption{真实港口规模算例下不同算法的时间与解质量对比}
\label{fig:member3-china-scale}
\end{figure}

结果表明，在该算例中 Gurobi 仍可作为商业求解器基准给出高质量解，但代表情景法能够用约四分之一的时间达到相同目标值。其原因是该方法优先保留高压力需求场景和需求分位点场景，较好地覆盖了影响一阶段投资决策的尾部风险。EV、均值指导启发式和场景分解的目标值比 Gurobi 高约 0.67\%，说明这些方法可以快速给出可行方案，但在高峰风险刻画方面略弱。列生成/场景生成也达到 Gurobi 基准目标值，但由于需要多轮“缩减场景求解--全场景评估”，本次算例中时间长于一次性代表情景法。

这一结果给出两个方法层面的结论。第一，在真实规模算例下，仅报告 Gurobi 结果是不够的，必须同时比较商业求解器与快速近似算法的时间和解质量。第二，对于政策仿真这类需要重复求解大量参数组合的任务，代表情景法更适合作为批量实验主力，而 Gurobi 完整 DEP 更适合作为基准校验工具。

\subsection{运营端政策灵敏度：补贴与碳价协同}

前述补贴实验只改变投资补贴比例 $k$，尚未直接量化运营端政策。为此，本节在真实港口规模算例上同时改变投资补贴比例 $k$ 与碳交易价格 $c_{\text{etp}}$，并将 EYC 作业占比作为运营端响应指标。表\ref{tab:member3-operational} 和图\ref{fig:member3-operational} 展示了关键结果。

\begin{table}[htbp]
\centering
\caption{投资补贴与碳交易价格的协同作用}
\label{tab:member3-operational}
\begin{tabular}{rrrrr}
\toprule
$k$ & 碳价 & 期望总排放 & EYC作业占比 & EYC投入 \\
\midrule
0.0 & 0 & 104893.3 & 0.00\% & 0.0 \\
0.0 & 600 & 104893.3 & 0.00\% & 0.0 \\
0.5 & 65 & 104893.3 & 0.00\% & 0.0 \\
0.5 & 300 & 97987.5 & 11.70\% & 27.1 \\
0.5 & 600 & 96556.0 & 14.13\% & 33.3 \\
1.0 & 0 & 59675.7 & 76.64\% & 450.2 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{../main/member3_operational_policy_sensitivity.png}
\caption{补贴比例与碳交易价格对排放、EYC 作业占比和 EYC 投入的影响}
\label{fig:member3-operational}
\end{figure}

实验结果支持“投资端激励与运营端约束需要协同”的判断。当 $k=0$ 时，即使碳价提高到 600，模型仍不选择 EYC 投资，EYC 作业占比为 0。这说明在设备购买和电网改造成本较高的真实规模算例中，单独依靠碳价未必足以跨过初始投资门槛。当 $k=0.5$ 且碳价仅为 65 时，结果同样没有变化；但当碳价提高到 300 和 600 时，EYC 作业占比分别提高到 11.70\% 和 14.13\%，总排放分别下降到 97987.5 和 96556.0 吨。这说明中等强度投资补贴需要与足够强的运营端碳价配合，才能真正改变作业结构。

此外，$k=1.0$ 在无碳价情形下也会触发大规模 EYC 投入和明显减排，但其投资补贴支出达到约 $2.41\times10^8$，政策成本很高。因此，从政府预算效率角度看，极端全额投资补贴并不是最优政策方向。更可取的路径是通过适度投资补贴降低前期门槛，再通过碳价或配额约束让企业在运营中持续使用 EYC 替代 DYC。

\subsection{政策组合验证}

管理洞察中提出了“投资补贴、碳交易价格、配额递减和绩效型补贴组合使用”的建议。为检验该建议，本节在真实规模算例上设置六类政策组合，并比较其排放、EYC 作业占比和相对无新增政策的减排率。

\begin{table}[htbp]
\centering
\caption{政策组合对运营排放的影响}
\label{tab:member3-policy-package}
\begin{tabular}{lrrr}
\toprule
政策组合 & 期望总排放 & EYC作业占比 & 减排率 \\
\midrule
无新增政策 & 104893.3 & 0.00\% & 0.00\% \\
仅投资补贴 & 104893.3 & 0.00\% & 0.00\% \\
仅碳价 & 104893.3 & 0.00\% & 0.00\% \\
补贴+碳价 & 97987.5 & 11.70\% & 6.58\% \\
补贴+碳价+配额递减 & 97728.6 & 12.14\% & 6.83\% \\
组合+绩效补贴 & 95522.3 & 15.88\% & 8.93\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{../main/member3_policy_package_validation.png}
\caption{单一政策与政策组合的减排效果比较}
\label{fig:member3-policy-package}
\end{figure}

表\ref{tab:member3-policy-package} 表明，在本算例中，仅采用中等强度投资补贴或仅提高碳价都不足以触发电动化运营替代，减排率为 0。将补贴与碳价结合后，EYC 作业占比提高到 11.70\%，总排放下降 6.58\%。进一步加入配额递减后，减排率小幅提高到 6.83\%；若再加入与 EYC 作业量挂钩的绩效型补贴，EYC 作业占比提高到 15.88\%，减排率提高到 8.93\%。

这一组实验直接验证了管理建议的有效性：单一政策工具往往只能作用于投资端或运营端的某一个环节，而港口设备电动化需要同时跨过“愿不愿意投”和“投了之后是否真正使用”的两道门槛。投资补贴降低固定成本，碳价和配额递减提高 DYC 运营代价，绩效型补贴则把财政支出与实际 EYC 作业量绑定。三者组合后，政策激励从设备购置传导到实际作业替代，因而比单一工具具有更稳定的减排效果。
...
\begin{itemize}[itemsep=0.2em,leftmargin=2em]
    ...
    \item 真实港口规模算例下，Gurobi 可作为基准求解器，代表情景法和列生成/场景生成可在保持解质量的同时支撑批量政策仿真。
    \item 单一投资补贴或单一碳价在中等强度下都可能无法触发减排，补贴与碳价协同后才明显提高 EYC 作业占比。
    \item 需求波动水平越高，企业越倾向于预留 EYC；但减排效果仍取决于 EYC 是否在运营中替代 DYC 作业。
    \item 补贴、碳价、配额递减和绩效型补贴的组合能够把政策激励从投资端传导到运营端，比单一政策具有更稳定的减排效果。
\end{itemize}

总体而言，本实验支持一个更细分的政策判断：投资补贴适合解决前期改造成本高、企业现金流压力大的问题；碳交易价格和排放配额规则更适合持续影响运营阶段的设备使用结构；绩效型补贴适合将财政支出与实际减排结果绑定。对于港口设备电动化改造，较优的政策设计不应只关注“补贴比例多高”，还应关注补贴是否能触发额外改造、是否能引导 EYC 被实际用于高排放替代场景，以及是否与碳市场信号形成一致激励。
